...
def main():
    ...
    if not viking.initialize():
        print("\n[WARN] OpenViking 未安装，使用 ChromaDB-only 模式")
        print("  如需完整静态知识，请安装 OpenViking")
        print("  现在将使用 ChromaDB 导入...")
        # 降级到 ChromaDB-only 导入脚本
        import importlib.util
        chroma_import_path = Path(__file__).parent / "import_to_chromadb.py"
        if chroma_import_path.exists():
            spec = importlib.util.spec_from_file_location("import_to_chromadb", chroma_import_path)
            chroma_mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(chroma_mod)
            return chroma_mod.main()
        else:
            print("[FAIL] ChromaDB 导入脚本未找到，请运行: python deepagent/knowledge/import_to_chromadb.py --all")
            return 1
