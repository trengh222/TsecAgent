...
def _build_llm():
    """从环境变量构建 ChatAnthropic 实例。

    优先级:
      API Key → ANTHROPIC_API_KEY > ANTHROPIC_AUTH_TOKEN
      Base URL → ANTHROPIC_BASE_URL（支持中转代理）
    """
    from langchain_anthropic import ChatAnthropic

    # strip() + 去掉可能由 linter 加入的引号，空值 fallback 到 AUTH_TOKEN
...
async def _check_terminal(executor) -> CheckResult:
    ...
    try:
        ...
        elapsed = time.monotonic() - t
        if r.get("success"):
            return CheckResult(_Status.OK, "Terminal Executor", r.get("stdout", r.get("output", "")).strip()[:40], elapsed, critical=True)
...
async def _check_recon(executor) -> CheckResult:
    t = time.monotonic()
    import shutil
...
def _build_tools(executors: dict) -> dict:
    ...
    return {
        ...
        # 侦察
        "recon_port_scan":      rc.port_scan,
        "recon_directory_scan": rc.directory_scan,
        # 知识库
...
async def main() -> None:
    ...
    try:
        ...
        print(f"\n[✗] LLM 构建失败: {e}")
        print("    请检查 .env 中 ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN")
        sys.exit(1)

    base_url = os.getenv("ANTHROPIC_BASE_URL", "https://api.anthropic.com")
    print(f"\n  模型   : {model_name}")
    print(f"  接入点 : {base_url}")
    print(f"  代理   : {config['proxy']}:{config['proxy']}")
    ...
    from deepagent.mcp.executors import (
        ...
        ProxyExecutor,  ReconExecutor,    KnowledgeExecutor,
    )
    executors = {
        ...
        "recon":     ReconExecutor(config["recon"]),
        ...
        if args.stream:
            ...
            if result.get("success"):
                ...
                if final_report:
                    ...
                    reflections = reflector.get("reflections", [])
                    if reflections:
                        last = reflections[-1]
                        print(f"\n  摘要: {last.get('summary', '')}")
