...
class VikingKnowledgeBackend:
    ...
    def initialize(self) -> bool:
        ...
        OpenViking = None
        try:
            from openviking_cli import OpenViking  # 旧版本 CLI
        except ImportError:
            pass

        if OpenViking is None:
            try:
                from openviking.sync_client import SyncOpenViking as OpenViking  # v0.4.x
            except ImportError:
                pass
        ...
        try:
            ...
            # 初始化客户端（适配新旧 API）
            # v0.4.x: SyncOpenViking(path=...)
            # 旧版: OpenViking(config_path=..., workspace=...)
            import inspect
            sig = inspect.signature(OpenViking.__init__)
            params = list(sig.parameters.keys())

            if 'path' in params:
                # 新版 API (v0.4.x)
                self._client = OpenViking(path=data_path)
            ...
            logger.info("OpenViking backend initialized", data_path=data_path,
