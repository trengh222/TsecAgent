...
async def _llm_invoke_with_retry(llm, prompt: str, max_retries: int = 4) -> str:
    ...
    _temp     = float(os.environ.get("LLM_TEMPERATURE", "0.7"))
...
━━━ 漏洞知识库（按漏洞类型读取 reference 获取 Payload/绕过/方法论，禁止凭印象编造 payload）━━━
针对具体漏洞类型构造 Payload/绕过前，先用 execute_shell 读取对应 reference 取实战 Payload 与绕过技巧，再 execute_python/execute_shell 执行；大文件（testing-methodology.md 589 行 / web-logic-auth.md 582 行）先 grep 关键词定位章节再局部读取，避免整文件加载。
目录: {_SECKNOWLEDGE_DIR}/references/  —— 命令模板: cat "<目录>/<文件>"
按漏洞类型选文件:
